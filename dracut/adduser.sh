...
echo voidbang > ${NEWROOT}/etc/hostname
...
chroot ${NEWROOT} useradd -m -c $USERNAME -G wheel,audio -s $USERSHELL $USERNAME
chroot ${NEWROOT} passwd -d $USERNAME >/dev/null 2>&1

# Setup default root/user password (voidlinux).
chroot ${NEWROOT} sh -c 'echo "root:voidbang" | chpasswd -c SHA512'
chroot ${NEWROOT} sh -c "echo "$USERNAME:voidbang" | chpasswd -c SHA512"
